...
- RequestCallbackModal: `src/components/RequestCallbackModal.tsx`
  ...
  iOS keyboard gap fix: `isMobile` → Drawer path (vaul) is what iPhones render — NOT DialogContent. `visualViewport` resize listener targets `scrollRef` (attached to Drawer's inner scroll div). On keyboard open (vv.height < 85% of window.innerHeight): sets `scrollRef.current.style.maxHeight = vv.height - 120 - 72 + 'px'`. On keyboard close: resets to `''`. Guard: iOS-only (`/iphone|ipad|ipod/i`), deps `[isMobile, open]`. Drawer scroll div: `overflow-y-scroll flex-1 min-h-0` + `WebkitOverflowScrolling: touch` + `ref={scrollRef}`. All text/tel inputs have `onFocus={scrollToInput}` (300ms `scrollIntoView nearest smooth`). DialogContent: `h-dvh sm:h-auto sm:max-h-[90dvh] top-0 translate-y-0 sm:top-1/2 sm:-translate-y-1/2 rounded-none sm:rounded-lg` — no ref, no visualViewport code.
